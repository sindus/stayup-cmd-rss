...
class TestUpsertProfile:
    def test_returns_id(self):
        ...
        result = upsert_profile(conn, "https://example.com/feed.xml")
        assert result == 7
        sql = cursor.execute.call_args[0][0]
        assert "INSERT INTO profile" in sql
        assert "ON CONFLICT" in sql

    def test_passes_url_as_parameter(self):
        conn, cursor = make_conn_mock()
        cursor.fetchone.return_value = (1,)
        upsert_profile(conn, "https://example.com/feed.xml")
...
class TestGetLatestEntry:
    ...
    def test_returns_tuple_when_row_exists(self):
        conn, cursor = make_conn_mock()
        cursor.fetchone.return_value = ("https://example.com/entry/1", '{"title": "Hello"}')
        version, content = get_latest_entry(conn, 1)
        assert version == "https://example.com/entry/1"
        assert content == '{"title": "Hello"}'

    def test_queries_by_provider_id(self):
        conn, cursor = make_conn_mock()
        cursor.fetchone.return_value = None
...
class TestSaveEntry:
    def test_inserts_with_version_and_commits(self):
        ...
        save_entry(conn, 1, "guid-123", '{"title": "test"}', None, None, executed_at)
        cursor.execute.assert_called_once()
        conn.commit.assert_called_once()
        params = cursor.execute.call_args[0][1]
        assert params[0] == 1
        assert params[1] == "guid-123"
        assert params[5] == executed_at

    def test_success_flag_in_sql(self):
        conn, cursor = make_conn_mock()
        save_entry(conn, 1, None, "{}", None, None, datetime.now(tz=timezone.utc))
        sql = cursor.execute.call_args[0][0]
        assert "TRUE" in sql
...
class TestFetchLatestEntry:
    @patch("fetch_rss.feedparser.parse")
    def test_returns_dict_with_correct_keys(self, mock_parse):
        ...
        result = fetch_latest_entry("https://example.com/feed.xml")
        assert result is not None
        assert result["version"] == "https://example.com/entry/1"
        assert result["title"] == "Hello World"
        assert result["link"] == "https://example.com/1"
        assert result["summary"] == "A summary."

    @patch("fetch_rss.feedparser.parse")
    def test_returns_none_when_no_entries(self, mock_parse):
    ...
    def test_parses_published_date(self, mock_parse):
        ...
        mock_parse.return_value = MagicMock(entries=[entry])
        result = fetch_latest_entry("https://example.com/feed.xml")
        assert result["published"] == datetime(2024, 6, 15, 0, 0, 0, tzinfo=timezone.utc)
    ...
    def test_handles_missing_id_falls_back_to_link(self, mock_parse):
        ...
        mock_parse.return_value = MagicMock(entries=[entry])
        result = fetch_latest_entry("https://example.com/feed.xml")
        assert result["version"] == "https://example.com/fallback"
